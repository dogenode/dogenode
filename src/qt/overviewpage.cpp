...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDogeNodeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDogeNodeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDogeNodeAmount = strAnonymizeDogeNodeAmount.remove(strAnonymizeDogeNodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDogeNodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDogeNodeAmount*COIN) nMaxToAnonymize = nAnonymizeDogeNodeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDogeNodeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDogeNodeAmount));
        strAnonymizeDogeNodeAmount = strAnonymizeDogeNodeAmount.remove(strAnonymizeDogeNodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDogeNodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
