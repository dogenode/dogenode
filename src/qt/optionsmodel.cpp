...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDogeNodeAmount"))
        settings.setValue("nAnonymizeDogeNodeAmount", 1000);
    nAnonymizeDogeNodeAmount = settings.value("nAnonymizeDogeNodeAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeDogeNodeAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeDogeNodeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeDogeNodeAmount:
            return QVariant(nAnonymizeDogeNodeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeDogeNodeAmount:
            nAnonymizeDogeNodeAmount = value.toInt();
            settings.setValue("nAnonymizeDogeNodeAmount", nAnonymizeDogeNodeAmount);
            emit AnonymizeDogeNodeAmountChanged(nAnonymizeDogeNodeAmount);
            break;
        default:
            break;
        }
