...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("DogeNode"))
        return false;
...
{
    // Convert DogeNode:// to DogeNode:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("DogeNode://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "DogeNode:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DogeNode.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for DogeNode.lnk
...
{
    return GetAutostartDir() / "DogeNode.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a DogeNode.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=DogeNode\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the DogeNode app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add DogeNode app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DogeNode-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  DogeNode-qt [" + tr("command-line options") + "]                     " + "\n";
