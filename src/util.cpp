...
int nAnonymizeDogeNodeAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DogeNode
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DogeNode
    // Mac: ~/Library/Application Support/DogeNode
    // Unix: ~/.DogeNode
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DogeNode";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "DogeNode.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "dogenoded.pid"));
