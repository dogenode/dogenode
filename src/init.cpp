...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: DogeNode.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: dogenoded.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizedogenodeamount=<n> " + _("Keep N DogeNode anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DogeNode is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DogeNode is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("DogeNode version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "DogeNode server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DogeNode") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart DogeNode to complete") << "\n";
    ...
    nAnonymizeDogeNodeAmount = GetArg("-anonymizedogenodeamount", 0);
    if(nAnonymizeDogeNodeAmount > 999999) nAnonymizeDogeNodeAmount = 999999;
    if(nAnonymizeDogeNodeAmount < 2) nAnonymizeDogeNodeAmount = 2;
    ...
    LogPrintf("Anonymize DogeNode Amount %d\n", nAnonymizeDogeNodeAmount);
